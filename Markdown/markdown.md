...
아래 사용법에 대한 글은 존 그루버가 마크다운을 번역한 [http://nolboo.github.io/blog/2013/09/07/john-gruber-markdown/](http://nolboo.github.io/blog/2013/09/07/john-gruber-markdown/) 글을 참고했습니다.

===========================================================================================
# 마크다운 사용법

마크다운(`.md`)에서는 엔터를 치면 자동으로 줄바꿈이 된다. 			
html에서의 `<br />`태그가 자동으로 삽입된다는 이야기다.
...
아래는 예시입니다.
타이틀 태그를 넣지 않아도 `<h1>` 태그를 제목으로 인식한다.
...
## 목록 list
순서없는 목록(ul)은 `*`를 넣고 쓰면 된다. 또는 `+` 와 `-` 등을 사용해도 같으 결과가 나온다.
